...
**This file is duplicated at [`.claude/skills/dinner-planner-api/SKILL.md`](../../../.claude/skills/dinner-planner-api/SKILL.md).** `.claude/skills/` is where Claude Code actually discovers and loads skills from; this `.agents/skills/` copy is kept identical per an earlier explicit request. Edit both together — they must stay byte-identical (aside from this note).
